(function(){
  ...
  QUnit.test("Inner scope variables override outter scope variables.", function(assert){
    ...
    function yay(){
      var peanuts = "roasted"; //this one overides the outerscoop while in the function

      assert.equal(peanuts, "roasted");
    }
    yay();

    assert.equal(peanuts, 300); //inner var is still scoped to function scoop and therefore can not reassign the outer var
  });
